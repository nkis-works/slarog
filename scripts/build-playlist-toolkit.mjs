...
const copy = {
  ...
  ja: {
    ...
    common: {
      ...
      independent:
        'NKIS Worksが開発する独立した支援アプリです。Amazonによる提供・公認ではなく、提携関係もありません。',
      footer:
        'ご利用には、対応バージョンのAmazon Music Android版が別途必要です。AmazonおよびAmazon Musicは各権利者の商標です。',
      back: '製品概要',
      contents: 'このページの内容',
      email: 'メールで問い合わせ',
      menu: 'メニュー',
    ...
    home: {
      ...
      maintenanceNote: 'Amazon Musicの画面変更に合わせて、アプリの更新で対応を進めます。',
      ...
      cardTitle: '購入前に、無料でチェック',
      cardBody: 'お使いの端末が対応しているか、購入前に確認できます。',
      trust: [
        ['曲の情報は端末内で処理', '曲名やプレイリスト名をNKIS Worksのサーバーへ送信しません。'],
        ['購入前に対応状況を確認', 'お使いのAmazon Musicが対応しているか、無料で確認できます。'],
        ['操作対象が不明なら停止', '必要なボタンやメニューを認識できない場合は、操作を中断します。'],
      ],
      featuresEyebrow: 'プレイリスト整理の主な機能',
      featuresTitle: 'いつものプレイリストを、もっと使いやすく。',
      featuresLead:
        '表示順の変更から重複候補の確認まで。Amazon Musicのプレイリストを整理する、日々の操作を支援します。',
      features: [
        [
          'ORDER',
          '表示順を、いつでもすぐに復元',
      ...
      flowTitle: '購入前に、機能と対応状況を確認。',
      flowLead:
        'まずは機能と月額料金をご確認ください。その後、画面操作支援を有効にして、無料の互換性チェックへ進みます。',
      steps: [
        ['機能と料金を確認', '初回画面で、主な機能と月額料金をご案内します。'],
        ['画面操作支援を有効にする', 'Accessibilityは説明したプレイリスト支援のためだけに使用します。'],
        ['無料互換性チェック', '見出し、曲一覧、並び替え、曲追加の操作項目を確認します。'],
        ['月額プランで利用を開始', '対応端末と確認できた場合だけGoogle Playの購入画面へ進めます。'],
      ],
      ...
      faqs: [
        [
          'アプリを閉じると、並び替えは元に戻りますか？',
          '「表示順の変更」と「保存した曲順の編集」では異なります。表示順の変更は一覧の見え方だけを切り替えるもので、Amazon Music側で元に戻ることがあります。Playlist Toolkitは好みの表示順を記憶し、再適用を支援します。一方、範囲移動や曲の追加をAmazon Musicで確定・保存した結果は、支援アプリを閉じても残ります。',
        ],
        [
          'Amazonの公式アプリですか？',
          'いいえ。NKIS Worksが開発する独立した支援アプリです。Amazonによる提供・公認ではありません。',
        ],
        [
          'Amazonのパスワードを入力する必要はありますか？',
          'Playlist Toolkitへの入力は不要です。Amazon Musicへのログインは、Amazon Musicアプリで行ってください。Playlist Toolkitがパスワードを求めたり、保存したりすることはありません。',
        ],
        [
          '画面操作支援を有効にすると、何ができますか？',
          'Amazon Musicに表示される曲名やボタンなどを読み取り、指定されたプレイリスト操作を支援します。利用する情報と操作の内容は、有効にする前にアプリ内で説明します。',
        ],
        [
          'Amazon Musicの更新後も使えますか？',
          '画面の変更内容によっては、一部の機能が使えなくなる場合があります。操作に必要な項目を認識できない場合は停止します。変更内容を調査し、アプリの更新で対応を進めます。',
        ],
        [
          'プレイリストの情報は外部に送信されますか？',
          '曲名やプレイリスト名など、整理に使う情報は端末内で処理し、NKIS Worksのサーバーへ送信しません。購入と契約状況の確認にはGoogle Playを利用します。',
        ],
      ],
      ctaTitle: 'お使いの端末で、まずは無料チェック。',
      ctaBody:
        '互換性チェックで、お使いのAmazon Musicが対応しているかを確認できます。チェックだけでは料金はかかりません。',
      ctaButton: '互換性ヘルプを見る',
    },
    privacy: {
      title: 'プライバシーポリシー',
    ...
    support: {
      title: 'サポート',
      intro: '使い始め方や、操作が止まったときの対処方法、月額プランの解約方法をご案内します。',
      updated: 'サポート窓口：slarog.app@gmail.com',
      sections: [
        [
          '問い合わせ前にご確認ください',
          [
            'Playlist Toolkit・Android・Amazon Musicのバージョン、アプリの表示言語、どの操作で問題が起きたかをお知らせください。Amazonのパスワードやカード情報は送らないでください。',
          ],
        ],
        [
          '互換性チェックが完了しない',
          [
            '自分で作成したプレイリストなど、編集できるものをAmazon Musicで開いてください。曲一覧や並び替え、曲の追加メニューが表示される画面で、もう一度チェックを行います。画面変更に未対応の場合は、アプリの更新をお待ちください。',
          ],
        ],
        [
          '操作が途中で止まった',
          [
            'ボタンを認識できない場合や、画面が切り替わった場合などに、誤操作を防ぐため停止します。途中まで移動・追加されていることもあるため、まずAmazon Musicの曲一覧を確認してください。対象と画面を確認したうえで、必要な操作だけを再実行してください。',
          ],
        ],
        [
          '画面操作支援を無効にする',
          [
            'Androidの「設定」から「ユーザー補助」を開き、Playlist Toolkitのサービスをオフにしてください。端末によって項目名が異なる場合があります。',
          ],
        ],
        [
          'サブスクリプションを管理・解約する',
          [
            'Google Playの「お支払いと定期購入」から「定期購入」を開き、Playlist Toolkitを選択してください。利用期限や請求内容は、Google Playの表示をご確認ください。アプリを削除するだけでは解約されません。',
          ],
        ],
        [
          'お問い合わせ',
          [
            'お問い合わせは下記のメールアドレスへお送りください。診断情報を添える場合は、アプリ内で内容を確認してからコピーし、メールに貼り付けてください。',
          ],
        ],
      ],
    },
...
const productRefinement = {
  ...
  ja: {
    ...
    metaDescription:
      'Amazon Musicのプレイリスト整理をAndroidで支援。好みの表示順を記憶し、重複候補の確認や曲の追加・範囲移動を手助けします。購入前の互換性チェックは無料です。',
    heroTitle: ['Amazon Musicの', 'プレイリスト整理を、', 'もっと手軽に。'],
    heroLead:
      '好みの表示順を記憶・復元。重複候補の確認や、追加する曲の整理、お気に入りのプレイリストへの移動を支援します。Amazon Musicアプリ自体は変更しません。',
    planTitle: 'ご購入前に、無料の互換性チェック。',
    planLines: [
      '互換性チェックだけでは料金はかかりません。Google Playで購入を確定すると、月額プランが始まります。',
      '月額200円・自動更新。解約はGoogle Playの「定期購入」から行えます。',
      '全機能の利用と、Amazon Musicの画面変更に応じた互換性の改善・更新を含みます。',
    ],
    features: [
      [
        '表示順',
        '好みの表示順を記憶して、再適用',
        'アーティスト順など4種類の表示順を記憶。案内に沿ってAmazon Musicの並び替えボタンを1回タップすると、好みの表示順を再適用します。保存された曲順は変えません。',
      ],
      [
        'まとめて確認',
        '曲数が多くても、まとめて確認',
        '曲一覧を読み取り、重複の候補やダウンロード状況、いいね、歌詞、音質をまとめます。確認できる項目はAmazon Musicの表示によって異なります。',
      ],
      [
        'SMART ADD',
        '追加する曲を、先に整理',
        'Smart Addで追加候補をまとめ、内容を確認してからAmazon Musicでの追加操作へ進めます。',
      ],
      [
        'すぐに戻る',
        'よく使うプレイリストへ戻る',
        '登録したプレイリストへ、少ない手順で移動できます。毎回ライブラリから探し直す手間を減らします。',
      ],
    ],
    steps: [
      [
        '機能と料金を確認',
        '初回画面で主な機能と月額料金をご案内します。この時点で料金は発生しません。',
      ],
      [
        '画面操作支援を有効にする',
        '画面の読み取りと操作に使うAndroidのユーザー補助機能（Accessibility）です。アプリ内の説明を確認してから有効にしてください。',
      ],
      [
        '無料で対応状況を確認',
        'Amazon Musicでプレイリストを開き、曲一覧や並び替えなどの必要な項目を確認します。',
      ],
      [
        '月額プランで利用を開始',
        '互換性チェックを通過したら、Google Playで価格と条件を確認して購入できます。',
      ],
    ],
    safetyEyebrow: '誤操作を防ぐための仕組み',
    safetyTitle: '必要なボタンが見つからないときは、自動で停止。',
    safetyLead:
      'Amazon Musicの画面にあるボタンやメニューを確認してから、指定された操作を行います。対象を特定できない場合は、誤操作を防ぐために停止します。',
    safety: [
      [
        'ボタンの位置や表示を確認',
        '画面上のボタンやメニューから操作対象を探します。位置が変わっても、対象を認識できれば操作を続けられます。',
      ],
      [
        '必要な項目を確認してから開始',
        '曲一覧や並び替えメニューなど、その操作に必要な項目がそろっているかを確認します。',
      ],
      [
        '操作対象が不明なときは停止',
        'ボタンが見つからない場合や、似た項目があって区別できない場合は、操作を中断します。',
      ],
      [
        'Amazon Musicの更新にも継続対応',
        '画面の変更で使えなくなった機能は、変更内容を調査し、アプリのアップデートで対応を進めます。対応までご利用いただけない場合があります。',
      ],
    ],
    searchEyebrow: '目的に合わせた使い方',
    searchTitle: 'Playlist Toolkitでできること',
    searchLead:
    ...
    searchItems: [
      ...
      [
        '重複候補をまとめて確認',
        '重複候補は自動で削除しません。ライブ版や別の録音など、残したい曲が含まれていないか確認してください。',
      ],
      [
        '連続した曲をまとめて移動',
        '画面上で範囲と移動先を指定します。移動後にAmazon Musicで保存すると、その曲順が残ります。',
      ],
      [
        'よく使うプレイリストへ戻る',
        'よく使うプレイリストを登録しておくと、毎回ライブラリから探し直す手間を減らせます。',
      ],
    ],
    operator: '運営者情報',
  },
...
const rangeMoveCopy = {
  ...
  ja: {
    eyebrow: '範囲移動',
    title: '続けて並ぶ曲を、まとめて移動。',
    body: '画面に表示されている曲から、移動する範囲の最初と最後の曲を選び、移動先を指定します。移動後は曲順を確認し、Amazon Musicで保存してください。',
